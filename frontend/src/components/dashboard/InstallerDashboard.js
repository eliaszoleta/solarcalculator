...
<script>
(function(){var o=null;window.addEventListener('message',function(e){if(!e.data||e.data.type!=='MSW_OPEN_REPORT'||o)return;o=document.createElement('div');o.style.cssText='position:fixed;inset:0;z-index:2147483647;background:rgba(0,0,0,0.6);display:flex;align-items:center;justify-content:center;padding:20px;box-sizing:border-box;';var m=document.createElement('div');m.style.cssText='background:white;width:100%;max-width:760px;height:90vh;border-radius:16px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 24px 64px rgba(0,0,0,0.4);';var h=document.createElement('div');h.style.cssText='padding:14px 20px;border-bottom:1px solid #e2e8f0;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;background:white;';var tl=document.createElement('span');tl.style.cssText='font-weight:700;font-size:15px;font-family:system-ui,sans-serif;color:#0f172a;';tl.textContent='Full Solar Report';var cb=document.createElement('button');cb.style.cssText='font-size:13px;color:#64748b;font-weight:600;background:#f1f5f9;border:1px solid #e2e8f0;border-radius:8px;cursor:pointer;padding:6px 14px;font-family:inherit;';cb.textContent='\u00d7 Close';cb.onclick=cl;h.appendChild(tl);h.appendChild(cb);var fr=document.createElement('iframe');fr.src=e.data.url;fr.style.cssText='flex:1;border:none;width:100%;';fr.scrolling='yes';m.appendChild(h);m.appendChild(fr);o.appendChild(m);o.onclick=function(ev){if(ev.target===o)cl();};document.body.appendChild(o);document.body.style.overflow='hidden';function cl(){if(o&&o.parentNode)o.parentNode.removeChild(o);document.body.style.overflow='';o=null;}});})();
<\/script>`;
