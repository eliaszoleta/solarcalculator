...
function ReportContent({ results, form, lead, installerConfig, onReset, embedded, popup }) {
  ...
  const primary = cta.primaryColor || '#1b4d3e';
        ...
        <div className="chart-wrapper">
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={chart} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              {paybackYear && <ReferenceLine x={paybackYear} stroke="#0f172a" strokeDasharray="4 2" label={{ value: 'Break-even', position: 'top', fontSize: 10, fill: '#0f172a' }} />}
        ...
        embedded ? (
          <div className="results-card cta-card-embed">
            <div style={{ textAlign: 'center', padding: '8px 0' }}>
              ...
              <p style={{ color: '#bfe3cf', fontSize: 14, marginBottom: 20, lineHeight: 1.6 }}>
                {cta.ctaSubtext || 'Our team will design a custom solar system — free, no obligation.'}
              </p>
              {cta.companyName && (
                <p style={{ color: '#a8c3b5', fontSize: 13, marginBottom: 16, fontWeight: 600 }}>{cta.companyName}</p>
              )}
              <a
                href={cta.ctaButtonUrl || (cta.ctaPhone ? `tel:${cta.ctaPhone}` : '#')}
                style={{ display: 'inline-block', padding: '14px 32px', background: 'linear-gradient(135deg, #16a34a, #15803d)', color: 'white', borderRadius: 10, fontWeight: 700, fontSize: 16, textDecoration: 'none', boxShadow: '0 4px 14px rgba(22,163,74,0.4)' }}
              >
                {cta.ctaButtonText || 'Contact Us'}
              </a>
              {cta.ctaPhone && <p style={{ color: '#a8c3b5', fontSize: 14, marginTop: 12 }}>{cta.ctaPhone}</p>}
            </div>
          </div>
        ) : (
          ...
          <div style={{ background: `linear-gradient(135deg, ${primary}, ${primaryDark})`, borderRadius: 16, padding: '20px 20px 18px', marginBottom: 16, textAlign: 'center', color: 'white' }}>
            ...
            <div style={{ fontSize: isCash ? 24 : 30, fontWeight: 800, lineHeight: 1.15, marginBottom: 6 }}>
              {isCash
                ...
                : <>Save <span style={{ color: '#86efac' }}>{fmtDollar(daySavings)}/mo</span> from day one</>
              }
            </div>
...
function PublicEmailCapture({ results, form, savings, system }) {
  ...
  return (
    <div style={{ textAlign: 'center', padding: '4px 0 8px' }}>
      ...
      <p style={{ color: '#bfe3cf', fontSize: 13, marginBottom: 16, lineHeight: 1.5 }}>
